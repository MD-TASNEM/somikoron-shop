...
export default function EditProductPage() {
  ...
  const { register, handleSubmit, formState: { errors }, watch, setValue, reset } = useForm({
    ...
    defaultValues: {
      ...
      specifications: {}, images: [], rating: 0, reviews: 0, seoTitle: '', seoDescription: '', tags: [],
    },
  });

  useEffect(() => {
    const checkSlug = async () => {
      if (!slug || !product) { setSlugError(null); return; }
      if (slug === product.slug) { setSlugError(null); return; }
      try {
        setCheckingSlug(true);
        const res = await fetch(`/api/products/check-slug?slug=${encodeURIComponent(slug)}&excludeId=${product._id}`);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (!productRes.ok) throw new Error(productRes.status === 404 ? 'Product not found' : 'Failed to fetch product');
        const productResult = await productRes.json();
        if (!productResult.success) throw new Error(productResult.message || 'Failed to fetch product');
        ...
        setFeaturesBn(p.featuresBn?.length > 0 ? p.featuresBn : ['']);
        setSpecifications(p.specifications || {});
        setTags(p.tags || []);
        reset({
          name: p.name, nameBn: p.nameBn || '', price: p.price, originalPrice: p.originalPrice || 0,
          category: p.category, badge: p.badge || '', stock: p.stock,
          description: p.description, descriptionBn: p.descriptionBn || '',
          features: p.features || [], featuresBn: p.featuresBn || [],
          specifications: p.specifications || {}, images: p.images || [],
          rating: p.rating || 0, reviews: p.reviews || 0,
          seoTitle: p.seoTitle || '', seoDescription: p.seoDescription || '', tags: p.tags || [],
        });
        const catRes = await fetch('/api/products/categories');
        const catResult = await catRes.json();
        if (catResult.success) setCategories(catResult.data.categories.map((c) => c.name));
      } catch (err) {
  ...
  const addSpecification = () => {
    const s = { ...specifications, [`spec${Object.keys(specifications).length + 1}`]: '' };
    setSpecifications(s);
  };

  const updateSpecification = (key, value) => {
    const s = { ...specifications, [key]: value }; setSpecifications(s); setValue('specifications', s);
  };

  const removeSpecification = (key) => {
    const s = { ...specifications }; delete s[key]; setSpecifications(s); setValue('specifications', s);
  };

  const addTag = (tag) => {
    if (tag.trim() && !tags.includes(tag.trim())) { const t = [...tags, tag.trim()]; setTags(t); setValue('tags', t); }
  };
  ...
  const onSubmit = async (data) => {
    try {
      ...
      const res = await fetch(`/api/products/${productId}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(productData) });
      if (!res.ok) { const r = await res.json(); throw new Error(r.message || 'Failed to update product'); }
      const result = await res.json();
      if (!result.success) throw new Error(result.message || 'Failed to update product');
      router.push('/admin/products?success=product-updated');
    } catch (err) {
    ...
    return (
      <div className="flex items-center justify-center min-h-96">
        <div className="text-center">
          ...
          <p className="text-lg text-dark-gray">Loading product...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="flex items-center justify-center min-h-96">
        <div className="text-center max-w-md">
          ...
          <h3 className="text-xl font-semibold text-secondary mb-2">Product Not Found</h3>
          <p className="text-dark-gray mb-6">{error || 'We could not find the product.'}</p>
          <Link href="/admin/products" className="btn btn-primary">Back to Products</Link>
        </div>
      </div>
    );
  }
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-dark-gray">Update product information</p>
          </div>
        </div>
        <Link href={`/products/${product.slug}`} target="_blank" className="btn btn-outline flex items-center space-x-2">
          <Eye className="h-4 w-4" /><span>View Product</span>
        </Link>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-1 space-y-6">
            <div className="bg-white rounded-xl shadow-lg p-6">
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-secondary mb-2">Main Image</label>
                  <div className="relative">
                    {previewImage ? (
            ...
            <button type="submit" disabled={saving || !!slugError} className="w-full btn btn-primary flex items-center justify-center space-x-2 disabled:opacity-50">
              {saving ? <><div className="animate-spin rounded-full h-4 w-4 border-2 border-white border-t-transparent"></div><span>Saving...</span></> : <><Save className="h-4 w-4" /><span>Save Changes</span></>}
            </button>
          </div>
        </div>
      </form>
