    ...
    const fetchProduct = async () => {
      try {
        ...
        const relatedResponse = await api.get(
          `/api/products?category=${response.data.category}`,
        );
        if (Array.isArray(relatedResponse.data.products)) {
          setRelatedProducts(
          ...
          setRelatedProducts(
            relatedResponse.data.filter((p) => p._id !== id).slice(0, 3),
          );
        } else {
          setRelatedProducts([]);
        }
    ...
    <div className="pb-20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <nav className="flex items-center gap-2 text-sm text-secondary/40 mb-8">
          ...
          <ChevronRight className="w-3 h-3" />
          <span className="text-secondary/80 font-medium truncate max-w-[200px]">
            {product.name}
          </span>
        </nav>
          ...
          >
            ...
            <div className="grid grid-cols-4 gap-4">
              {[1, 2, 3, 4].map((i) => (
                <div
                  key={i}
                  className={`aspect-square rounded-subtle overflow-hidden bg-surface-container-low cursor-pointer hover:opacity-80 transition-opacity ${i === 1 ? "border-2 border-primary" : ""}`}
                >
                  <img
                    src={product.image}
                    alt={`${product.name} ${i}`}
            ...
            <div className="space-y-6 pb-8 border-b border-secondary/5">
                ...
                <div className="flex items-center gap-2">
                  <div className="flex items-center gap-1">
                    {[...Array(5)].map((_, i) => (
                      <Star
                        key={i}
                        className={`w-4 h-4 ${i < Math.floor(product.rating || 5) ? "fill-tertiary text-tertiary" : "text-secondary/20"}`}
                      />
                    ))}
                  </div>
                  <span className="text-sm font-bold text-secondary/60">
                    {product.rating || 5.0} (120 Reviews)
                  </span>
                </div>
                <h1 className="text-4xl md:text-5xl font-bold text-secondary leading-tight">
                  {product.name}
                </h1>
                <p className="bengali text-2xl text-primary font-bold">
                  {product.nameBn}
                </p>
              </div>
              ...
              <div className="flex items-center gap-4">
                ...
                {product.originalPrice && (
                  <span className="text-xl text-secondary/30 line-through">
                    ৳{product.originalPrice}
                  </span>
                )}
                {product.originalPrice && (
            ...
            <div className="py-8 space-y-6">
              {product.variants && product.variants.length > 0 && (
                <div className="space-y-4">
                  <p className="text-sm font-bold uppercase tracking-widest text-secondary/40">
                    Select Variant
                  </p>
