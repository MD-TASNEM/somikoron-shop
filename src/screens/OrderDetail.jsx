...
import { ArrowLeft, Package, Truck, CheckCircle, Clock, MapPin, CreditCard, ShoppingBag } from 'lucide-react';
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-12">
      <div className="flex items-center gap-4 mb-12">
        <Link to="/orders" className="p-2 hover:bg-secondary/5 rounded-full transition-colors">
                ...
                return (
                  <div key={step.status} className="flex flex-col items-center gap-3">
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center transition-all ${
                      isActive ? 'bg-primary text-white shadow-lg shadow-primary/20 scale-110' : 'bg-white border-2 border-secondary/5 text-secondary/20'
                    }`}>
                ...
                <div key={item.cartItemId || item.id} className="flex gap-6 group">
                  ...
                  <div className="flex-grow flex flex-col justify-center">
                    ...
                    <div className="flex items-center gap-4 mt-2">
                      ...
                      <p className="text-xs text-secondary/40 font-bold uppercase tracking-widest">Price: ৳{item.price}</p>
                    </div>
                    <p className="text-primary font-bold mt-1">Total: ৳{item.price * item.quantity}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="space-y-8">
          ...
          <section className="bg-white p-8 rounded-premium shadow-sm border border-secondary/5 space-y-6">
            ...
            <div className="space-y-2">
              ...
              <p className="text-sm text-secondary/60 font-bold">{order.formData.area}</p>
              <p className="text-sm font-bold text-primary mt-4">{order.formData.phone}</p>
            </div>
          </section>
          ...
          <section className="bg-white p-8 rounded-premium shadow-sm border border-secondary/5 space-y-6">
            ...
            <div className="p-4 bg-secondary/5 rounded-xl">
              <p className="text-xs font-bold text-secondary/40 uppercase tracking-widest">Method</p>
